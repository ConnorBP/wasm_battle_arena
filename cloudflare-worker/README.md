...
This Worker uses one Durable Object per versioned matchmaking room. It only pairs two browsers and relays WebRTC SDP/ICE; game packets remain peer-to-peer.
...
4. If the game host is Cloudflare-proxied, route the Worker under `/match/*`. Otherwise compile the game with `GHOST_BATTLE_SIGNALING_URL=wss://your-worker.workers.dev/match`.
