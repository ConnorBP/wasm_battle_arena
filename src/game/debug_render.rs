...
use super::{
    ...
    player::{grid_to_world, world_to_grid},
};
...
pub fn spawn_debug_sprites(mut commands: Commands, mut dbg_ents: ResMut<DebugEntitiesList>) {
                ...
                .spawn((
                    ...
                    SpriteBundle {
                        transform: Transform::from_translation(
                            grid_to_world((i % 3 as u32, i / 3 as u32)).extend(-1.),
                        ),
                        sprite: Sprite {
...
pub fn update_debug_sprites(
    ...
    for (transform, movedir) in &players {
        ...
        let h_block =
            world_to_grid(transform.translation.xy() + Vec2::new(movedir.0.x.signum(), 0.0));
        let v_block =
            world_to_grid(transform.translation.xy() + Vec2::new(0.0, movedir.0.y.signum()));
        let hv_block = world_to_grid(
            transform.translation.xy() + Vec2::new(movedir.0.x.signum(), movedir.0.y.signum()),
        );
        let h2_block = world_to_grid(
            transform.translation.xy() + Vec2::new(movedir.0.x.signum(), -movedir.0.y.signum()),
        );
        let v2_block = world_to_grid(
            transform.translation.xy() + Vec2::new(-movedir.0.x.signum(), movedir.0.y.signum()),
        );
